...
export function handleSummary(data) {
  ...
  // --- RPS ---
  let rpsQuery = `rate(workload_ops_total{type="cpu_go"}[${TEST_INTERVAL}])`;
  const rpsRes = queryPrometheus(rpsQuery);
  const rps = rpsRes && rpsRes.length > 0 ? parseFloat(rpsRes[0].value[1]) : 0;
  console.log(`RPS: ${rps.toFixed(2)}`);

  // --- p95 latency ---
  let p95Query = `histogram_quantile(0.90, rate(workload_latency_seconds_bucket{type="cpu_go"}[${TEST_INTERVAL}]))`;
